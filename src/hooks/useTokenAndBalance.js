import { tokens as tokenlist } from "../abi/tokenlist.json";
...
import { multicall2Address } from "../constant/constant";
...
const useTokensAndBalance = (address) => {
  ...
  const tokenAddresses = useMemo(() => mainnetTokens.map((x) => x.address), []);
  ...
  useEffect(() => {
    (async () => {
      const provider = new JsonRpcProvider(import.meta.env.VITE_MAINNET_RPC);
